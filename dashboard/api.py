...
def add_social_campaign(req: AddSocialCampaignRequest, _admin: Dict[str, Any] = Depends(require_admin)):
    ...
    scheduled_posts = []
    # Caption templates are keyed on the theme found in the keyword, so the
    # same theme always produced the same text — scheduling one airport
    # keyword this week and another next week posted identical copy to the
    # same account. The chain is unchanged; it is just callable now, so a
    # caption already used for this site and platform can step to the next
    # theme instead of going out twice.
    def _caption_for(kw_lower, plat_lower):
    ...
    # Probes that select each theme in the chain above, preferred one first.
    THEME_PROBES = ["airport", "corporate", "wine", "event", ""]

    used_captions = {
        (str(p.get("platform", "")).lower(), str(p.get("caption", "")))
    ...
    for idx, (kw, platform, sched_time_str, week_num) in enumerate(raw_posts):
        ...
        caption, hashtags = _caption_for(kw_lower, plat_lower)
        if (plat_lower, caption) in used_captions:
            for probe in THEME_PROBES:
                alt_caption, alt_hashtags = _caption_for(probe, plat_lower)
                if (plat_lower, alt_caption) not in used_captions:
                    caption, hashtags = alt_caption, alt_hashtags
                    break
        used_captions.add((plat_lower, caption))
