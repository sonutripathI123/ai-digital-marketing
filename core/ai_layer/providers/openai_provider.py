...
Reads OPENAI_API_KEY securely from environment or runtime parameter.
"""
...
class OpenAIProvider(BaseAIProvider):
    ...
    def generate(self, request: LLMRequest, model_override: Optional[str] = None) -> LLMResponse:
        ...
        model_name = model_override or request.preferred_model or "gpt-4o"
        ...
        if not self._api_key:
            return LLMResponse(
                ...
                success=False,
                error_message="OPENAI_API_KEY is not configured in environment. Please add it via the AI Vault.",
                latency_ms=(time.time() - start_time) * 1000
            )

        try:
            # Live OpenAI SDK invocation if package installed
            ...
            with urllib.request.urlopen(req, timeout=30) as resp:
                ...
                pricing = TOKEN_PRICING.get(model_name, {"input": 0.0025, "output": 0.010})
